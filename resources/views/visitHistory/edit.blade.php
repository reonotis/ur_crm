    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="visit-history-contents">
                <div class="card-body">
                    <form action="{{route('visitHistory.update', ['reserve_info' => $reserve_info->id ])}}"
                        ...
                        <div class="visit-history-row">
                            ...
                            <div
                                class="visit-history-content">{{ $reserve_info->customer->f_name . " " . $reserve_info->customer->l_name }}
                                &nbsp;様
                            </div>
                        </div>
                        <div class="visit-history-row">
                            ...
                            <div
                                class="visit-history-content">{{ $reserve_info->vis_date->format('Y 年 m 月 d 日') }}</div>
                        </div>
                        <div class="visit-history-row">
                            <div class="visit-history-title"><label for="vis_time">時間</label></div>
                            <div class="visit-history-content">
                                <input type="time" name="vis_time" class="form-control" id="vis_time"
                                       value="{{ substr($reserve_info->vis_time, 0, 5) }}">
                            </div>
                        </div>
                        <div class="visit-history-row">
                            <div class="visit-history-title"><label for="vis_time">セクション</label></div>
                            <div class="visit-history-content">
                                <div style="width: 100%">
                            ...
                            <div class="visit-history-content">
                                <select name="menu_id" id="menu_id" class="form-control">
                                    ...
                                    @foreach ($menus AS $menu)
                                        <option
                                            value="{{ $menu->id }}" {{ ($reserve_info->menu_id == $menu->id)? "selected": "" }} >{{ $menu->menu_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="visit-history-content">
                                <select name="user_id" id="user_id" class="form-control">
                                    @foreach ($users AS $user)
                                        <option
                                            value="{{ $user->id }}" {{ ($reserve_info->user_id == $user->id)? "selected": "" }} >{{ $user->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="visit-history-row">
                            ...
                            <div class="visit-history-content">
                                <textarea name="memo" id="memo"
                                          class="form-control">{{ $reserve_info->memo }}</textarea>
                            </div>
                        </div>
                        <div class="visit-history-row">
                                    ...
                                    @for ($angle = 1; $angle <= 3; $angle++)
                                        <div class="img-upload-box">
                                            ...
                                            <label class="img-upload-label">
                                                <input type="file" name="image[{{ $angle }}]" class="img-file"
                                                       id="img-file{{ $angle }}" accept=".png, .jpg, .jpeg">ファイルを選択
                                            </label>
                                            ...
                                            @foreach($images as $image)
                                                @if($image['angle'] == $angle)
                                                    <div class="customer-image-box">
                                                        <img id="img{{ $angle }}"
                                                             src="{{ asset('storage/' . Common::DISPLAY_CUSTOMER_IMG_RESIZE_DIR . '/' . $image['img_pass']) }}"
                                                             alt="{{ Common::ANGLE_LIST[$image['angle']] }}">
                                                    </div>
                                                    <div class="">
                                                        <label>
                                                            <input type="checkbox" name="imgDelete[{{ $angle }}]"
                                                                   class="img-delete" id="img-file{{ $angle }}-delete">
                                                            削除する
                                                        </label>
                                                    </div>
                                                @endif
